...
def seed_sale_with_receipt(db):
    ...
    bank_account = models.BankAccount(id=1, account_id=1, bank_name="测试银行", account_number="123456", balance=Decimal("11300"))
    db.add(bank_account)

    sale = models.SaleOrder(id=100, account_id=1, order_no="SO-001", total_price=Decimal("11300"), payment_status="paid", status="completed", sale_date=datetime(2026, 1, 15))
    db.add(sale)

    receipt = models.Receipt(id=1, account_id=1, receipt_type="sale", related_entity_type="sale_order", related_entity_id=100, amount=Decimal("11300"), receipt_method="company", receipt_date=datetime(2026, 1, 16), bank_account_id=1, bank_transaction_id=1)
    db.add(receipt)

    bank_tx = models.BankTransaction(id=1, account_id=1, bank_account_id=1, transaction_type="inflow", amount=Decimal("11300"), balance_after=Decimal("11300"), transaction_date=datetime(2026, 1, 16), description="收款", related_entity_type="receipt", related_entity_id=1)
    db.add(bank_tx)
...
def seed_purchase_with_payment(db):
    ...
    bank_account = models.BankAccount(id=1, account_id=1, bank_name="测试银行", account_number="123456", balance=Decimal("0"))
    db.add(bank_account)

    purchase = models.PurchaseOrder(id=200, account_id=1, order_no="PO-001", total_price=Decimal("5650"), payment_status="paid", status="completed", purchase_date=datetime(2026, 2, 1))
    db.add(purchase)

    payment = models.Payment(id=1, account_id=1, payment_type="purchase", related_entity_type="purchase_order", related_entity_id=200, amount=Decimal("5650"), payment_method="company", payment_date=datetime(2026, 2, 2), bank_account_id=1, bank_transaction_id=2)
    db.add(payment)

    bank_tx = models.BankTransaction(id=2, account_id=1, bank_account_id=1, transaction_type="outflow", amount=Decimal("5650"), balance_after=Decimal("0"), transaction_date=datetime(2026, 2, 2), description="付款", related_entity_type="payment", related_entity_id=1)
    db.add(bank_tx)
...
class TestReverseReceipts:
    def test_creates_reversal_receipt(self, db, seed_sale_with_receipt):
        ...
        reversal = [r for r in receipts if r.amount < 0]
        assert len(reversal) == 1
        assert reversal[0].amount == Decimal("-11300")
...
class TestReversePayments:
    def test_creates_reversal_payment(self, db, seed_purchase_with_payment):
        ...
        reversal = [p for p in payments if p.amount < 0]
        assert len(reversal) == 1
        assert reversal[0].amount == Decimal("-5650")
    ...
    def test_reverses_bank_balance(self, db, seed_purchase_with_payment):
        ...
        reverse_payments(db, 1, 200)
        bank_account = db.query(models.BankAccount).get(1)
        assert bank_account.balance == Decimal("5650")
