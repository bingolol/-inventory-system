...
| 通用(9xxx) | `VALIDATION_ERROR`, `DUPLICATE_ENTRY`, `INTERNAL_ERROR`, `ENDPOINT_NOT_ALLOWED_FOR_AI` |
...
### 4.3 `AccountingError`（models_finance.py:17）
LedgerEngine / JournalEngine / ReceivableEngine 内部使用，纯字符串 code。通过 `finance_integration.py` 的 `post_journal()`/`reverse_journal()` 转为 `BusinessError`。
| string code | 触发引擎 | 说明 |
|-------------|----------|------|
