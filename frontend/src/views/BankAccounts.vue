    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
      <el-select v-model="currentAccountId" placeholder="选择银行账户" style="width:240px;" @change="onAccountChange">
        <el-option v-for="acc in accounts" :key="acc.id" :label="`${acc.bank_name} (${maskAccount(acc.account_number)})`" :value="acc.id">
          <div style="display:flex;justify-content:space-between;align-items:center;width:100%;"><span>{{ acc.bank_name }}</span><span style="font-weight:600;color:var(--primary);">{{ formatMoney(acc.balance) }}</span></div>
        </el-option>
      </el-select>
...
.cl { font-size:12px; color:var(--text-regular); font-weight:500; margin-bottom:4px; }
.cv { font-size:24px; font-weight:700; letter-spacing:-0.5px; margin-bottom:2px; }
.cs { font-size:12px; color:var(--text-secondary); }
