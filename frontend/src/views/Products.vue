  ...
  <div>
    <el-card shadow="never">
      ...
      <FilterBar @search="loadData" @reset="resetFilters">
        <el-input v-model="filters.search" placeholder="搜索名称/编码" clearable style="width:220px" @clear="loadData" @keyup.enter="loadData">
          <template #prefix><el-icon><Search /></el-icon></template>
        </el-input>
        <el-input v-model="filters.sku" placeholder="SKU精确匹配" clearable style="width:160px" @clear="loadData" @keyup.enter="loadData" />
...
const { list, loading, filters, pagination, loadData, reset } = useList({
  api: productsApi,
  method: 'getProducts',
  defaultFilters: { search: '', sku: '', category: '' },
  buildParams: (f) => {
    const params = {}
  ...
  onError: (e) => handleError(e, { defaultMsg: '加载商品列表失败，请检查网络连接' })
})
