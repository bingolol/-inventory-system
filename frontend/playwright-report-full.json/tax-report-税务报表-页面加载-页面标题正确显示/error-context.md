...
- Name: tax-report.spec.js >> 税务报表 >> 页面加载 >> 页面标题正确显示
- Location: tests\e2e\tax-report.spec.js:11:5
...
Expected substring: "税务报表"
Error: strict mode violation: locator('.page-title') resolved to 4 elements:
    1) <span class="page-title">期末税务</span> aka getByRole('main').getByText('期末税务')
    2) <span data-v-e564dcae="" class="page-title">税务报表</span> aka getByLabel('税务报表').getByText('税务报表')
    3) <span class="page-title">附加税申报</span> aka getByLabel('附加税申报').getByText('附加税申报', { exact: true })
    4) <span class="page-title">期末处理</span> aka getByLabel('期末处理').getByText('期末处理')
