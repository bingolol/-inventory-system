...
- Name: expenses.spec.js >> 费用管理 >> 列表展示 >> 费用列表正确展示后端数据
- Location: tests\e2e\expenses.spec.js:12:5
...
Locator: locator('th:has-text("日期")')
Expected: visible
Error: strict mode violation: locator('th:has-text("日期")') resolved to 2 elements:
    1) <th colspan="1" rowspan="1" scope="col" class="el-table_1_column_1 is-leaf el-table__cell">…</th> aka getByRole('columnheader', { name: '日期' })
    2) <th colspan="1" rowspan="1" scope="col" class="el-table_2_column_10 is-leaf el-table__cell">…</th> aka getByLabel('个人垫付', { exact: true }).locator('th').filter({ hasText: '日期' })

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for locator('th:has-text("日期")')
