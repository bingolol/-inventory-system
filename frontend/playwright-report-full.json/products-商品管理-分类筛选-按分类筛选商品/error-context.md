...
- Name: products.spec.js >> 商品管理 >> 分类筛选 >> 按分类筛选商品
- Location: tests\e2e\products.spec.js:132:5

# Error details

```
Error: locator.click: Error: strict mode violation: locator('.filter-bar .el-select') resolved to 2 elements:
    1) <div class="el-select" data-v-a03b6bb9="">…</div> aka locator('div').filter({ hasText: /^分类筛选$/ }).nth(1)
    2) <div class="el-select">…</div> aka locator('div').filter({ hasText: /^分类筛选$/ }).nth(5)

Call log:
  - waiting for locator('.filter-bar .el-select')
