...
- Name: customers.spec.js >> 客户管理 >> 删除客户 >> 取消删除不执行
- Location: tests\e2e\customers.spec.js:285:5

# Error details

```
Error: locator.click: Error: strict mode violation: locator('.el-table__row:first-child button:has-text("删除")') resolved to 2 elements:
    1) <button type="button" data-v-14585336="" aria-disabled="false" class="el-button el-button--danger el-button--small is-link el-tooltip__trigger">…</button> aka getByRole('button', { name: '删除' }).first()
    2) <button type="button" data-v-14585336="" aria-disabled="false" class="el-button el-button--danger el-button--small is-link el-tooltip__trigger">…</button> aka locator('button').filter({ hasText: '删除' }).nth(2)
