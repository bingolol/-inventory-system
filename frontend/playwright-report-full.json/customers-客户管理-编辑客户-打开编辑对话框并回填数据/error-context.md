...
- Name: customers.spec.js >> 客户管理 >> 编辑客户 >> 打开编辑对话框并回填数据
- Location: tests\e2e\customers.spec.js:193:5

# Error details

```
Error: locator.click: Error: strict mode violation: locator('.el-table__row:first-child button:has-text("编辑")') resolved to 2 elements:
    1) <button type="button" data-v-14585336="" aria-disabled="false" class="el-button el-button--primary el-button--small is-link">…</button> aka getByRole('button', { name: '编辑' }).first()
    2) <button type="button" data-v-14585336="" aria-disabled="false" class="el-button el-button--primary el-button--small is-link">…</button> aka locator('button').filter({ hasText: '编辑' }).nth(2)

Call log:
  - waiting for locator('.el-table__row:first-child button:has-text("编辑")')
