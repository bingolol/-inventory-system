...
- Name: tax-report.spec.js >> 税务报表 >> 页面加载 >> 默认选中增值税报表标签页
- Location: tests\e2e\tax-report.spec.js:20:5
...
Expected substring: "增值税报表"
Error: strict mode violation: locator('.el-tabs__item.is-active') resolved to 2 elements:
    1) <div role="tab" id="tab-tax" tabindex="0" aria-selected="true" aria-controls="pane-tax" class="el-tabs__item is-top is-active">…</div> aka getByRole('tab', { name: '税务报表' })
    2) <div role="tab" id="tab-vat" tabindex="0" aria-selected="true" aria-controls="pane-vat" class="el-tabs__item is-top is-active">…</div> aka getByRole('tab', { name: '增值税报表' })
