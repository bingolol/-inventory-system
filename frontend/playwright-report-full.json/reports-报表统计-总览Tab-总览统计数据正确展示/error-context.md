...
- Name: reports.spec.js >> 报表统计 >> 总览Tab >> 总览统计数据正确展示
- Location: tests\e2e\reports.spec.js:34:5
...
Locator: locator('.el-tab-pane').first().locator('.stat-label:has-text("商品种类")')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.el-tab-pane').first().locator('.stat-label:has-text("商品种类")')
