...
def map_vat_to_main_form(
    ...
    Args:
        ...
        profile: 主体画像
        input_tax: 当期进项税额
        carry_forward: 上期期末留抵税额
    """
    deductible_total = input_tax + carry_forward
    ...
    rows = {
        "row_1": float(result.total_revenue),
        "row_2": float(result.total_revenue),
        "row_3": 0.0,
        "row_4": 0.0,
        "row_5": 0.0,
        "row_11": float(result.tax_payable_gross),
        "row_12": float(input_tax),
        "row_13": float(carry_forward),
        "row_14": 0.0,
