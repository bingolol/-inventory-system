...
def get_profit_report(db: Session, account_id: int, start_date: str = None, end_date: str = None):
    ...
    total_revenue_l1 = _d(sum(Decimal(str(o.total_price_l1 or 0)) for o in sale_orders))
    ...
    total_profit = total_revenue_l1 - total_cost

    return {
        "total_sale_amount": round(float(total_revenue_l1), 2),
        "total_profit": round(float(total_profit), 2),
        "sale_count": len(sale_orders),
        "sale_orders": sale_orders,
        "total_revenue_l1": round(float(total_revenue_l1), 2),
