...
def list_products(db: Session, account_id: int, skip: int = 0, limit: int = 100, search: str = None, sku: str = None, category: str = None):
    q = db.query(models.Product).filter(models.Product.account_id == account_id)
    if search:
...
def create_product(db: Session, account_id: int, data: schemas.ProductCreate):
    ...
    db.add(inv)
    _log(db, account_id, "create", "product", product.id, f"创建商品: {product.name} (SKU:{product.sku})")
    db.flush()
    return product


def update_product(db: Session, account_id: int, product_id: int, data: schemas.ProductUpdate):
    ...
    for k, v in changes.items():
        setattr(product, k, v)
    _log(db, account_id, "update", "product", product_id, f"更新商品: {product.name}")
    db.flush()
    return product


def delete_product(db: Session, account_id: int, product_id: int):
    product = get_product(db, account_id, product_id)
    if not product:
        return False
    ...
    _log(db, account_id, "delete", "product", product_id, f"删除商品: {product.name}")
...
def adjust_inventory(db: Session, account_id: int, product_id: int, data: schemas.InventoryAdjust):
    inv = get_or_create_inventory(db, account_id, product_id)
    old_qty = inv.quantity
    inv.quantity = data.quantity
    _log(db, account_id, "adjust", "inventory", product_id, f"库存盘点: {old_qty}->{data.quantity}")
    db.flush()
    return inv
