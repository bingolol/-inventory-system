...
def _build_ob_out(opening_balance):
    ...
    return {
        ...
        "date": opening_balance.date_l1.isoformat() if opening_balance.date_l1 else None,
        "cash_balance": opening_balance.cash_balance_l1,
        "bank_balance": opening_balance.bank_balance_l1,
        "accounts_receivable": opening_balance.accounts_receivable_l1,
        "inventory_value": opening_balance.inventory_value_l1,
        "fixed_assets_original": opening_balance.fixed_assets_original_l1,
        "accumulated_depreciation": opening_balance.accumulated_depreciation_l1,
        "intangible_assets_original": opening_balance.intangible_assets_original_l1,
        "accumulated_amortization": opening_balance.accumulated_amortization_l1,
        "accounts_payable": opening_balance.accounts_payable_l1,
        "tax_payable": opening_balance.tax_payable_l1,
        "long_term_borrowings": opening_balance.long_term_borrowings_l1,
        "paid_in_capital": opening_balance.paid_in_capital_l1,
        "retained_earnings": opening_balance.retained_earnings_l1,
...
def create_opening_balance(data: schemas.OpeningBalanceCreate, account_id: int = Depends(get_account_id), operator: str = Depends(get_operator), db: Session = Depends(get_db)):
    ...
    result = OperationResult(
        ...
        summary=f"期初余额创建成功，日期 {opening_balance.date_l1}",
