...
> **Agent 记忆系统的具体位置因平台而异**（Claude Code 在 `~/.claude/projects/<...>/memory/`，Codex 用 `AGENTS.md`，OpenCode 用 `.opencode/`，OpenClaw 用 `~/.openclaw/`）。完整路径速查见 [references/agent-paths.md](/KKKKhazix/khazix-skills/blob/main/neat-freak/references/agent-paths.md)。如果当前 agent 没有独立的记忆系统，直接跳过这一层，把功夫全花在 docs 和项目根 markdown 上。
...
完整映射表（覆盖更多变更类型与对应文档）见 **[references/sync-matrix.md](/KKKKhazix/khazix-skills/blob/main/neat-freak/references/sync-matrix.md)**——遇到不确定的改动先查这张表。
...
-   **[references/sync-matrix.md](/KKKKhazix/khazix-skills/blob/main/neat-freak/references/sync-matrix.md)** — 完整的"变更类型 → 要改哪些文件"映射表
-   **[references/agent-paths.md](/KKKKhazix/khazix-skills/blob/main/neat-freak/references/agent-paths.md)** — Claude Code / Codex / OpenCode 各自的记忆与配置路径速查